...
def garantir_diretorio_exportacao(nome_pasta: str = "noticias_prontas") -> Path:
    ...
    try:
        ...
        print("\n" + "!" * 80)
        print(f"[AVISO DE PERMISSÃO] O sistema operacional impediu a criação automática da pasta '{nome_pasta}'.")
        print(f"Caminho esperado: {pasta_destino.resolve()}")
        print("Solução rápida: Crie uma pasta chamada 'noticias_prontas' manualmente neste diretório e tente novamente.")
        print("!" * 80 + "\n")
        sys.exit(0)
...
def gerar_conteudo_markdown(artigo: Dict[str, str], slug: str) -> str:
    """
    Monta a estrutura interna do arquivo Markdown com Título, Imagem Placeholder, Resumo e Fonte.
    
    Args:
        artigo (Dict[str, str]): Dados da notícia curada pela IA.
        slug (str): Identificador único usado como semente para a imagem.
    ...
    # Placeholder de imagem dinâmico e gratuito baseado no seed do slug
    url_imagem = f"https://picsum.photos/seed/{slug}/800/400"
...
def exportar_noticias_para_markdown(
    ...
    pasta_destino: str = "noticias_prontas"
) -> List[Path]:
    """
...
def executar_fase3():
    ...
    print("\n" + "=" * 80)
    print(f"[OK] Fase 3 concluída com sucesso! {len(arquivos)} arquivo(s) .md gerado(s).")
